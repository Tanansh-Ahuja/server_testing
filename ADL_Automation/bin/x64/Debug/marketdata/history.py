...
class history:
    ...
    def __init__(self, _symbol, store_all_ticks=True, save_history_to_files=True):
        ...
        self.symbol = _symbol
        self.save_history_to_files = False  # Force disable file writing
        ...
        # Path(self.HISTORY_DIR).mkdir(parents=True, exist_ok=True)  # Disable directory creation

        self.HISTORY_FILE = f"{self.symbol.replace('/', '')}.log"
        self.HISTORY_FILE_TOB = f"{self.symbol.replace('/', '')}_TOB.log"

        self.history_logger = None  # Disable logger
        self.history_tob_logger = None  # Disable logger
    ...
    def _update_asset(self, date_time, _symbol, depth, bid, ask, bid_size, ask_size):
        ...
        if (
            self.BID[depth] is None
            or self.ASK[depth] is None
            or self.BID_SIZE[depth] is None
            or self.ASK_SIZE[depth] is None
        ):
            return
        ...
        if self.store_all_ticks:
            ...
            try:
                ...
                if new_tob_bid or new_tob_ask:
                    self.HISTORY_TOB.append({"date_time": date_time, "bid": self.BID_TOB, "ask": self.ASK_TOB})
